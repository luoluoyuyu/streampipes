  ...
  public void run() {
    synchronized (ResourceUnitMigration.class) {
      try {
        ...
        registeredServices.forEach(a -> checkServiceHealth(a));
        for (SpServiceRegistration service : serviceRegistrations) {
          System.out.println(service.getServiceUrl());
          List<ResourceUnit<InvocableStreamPipesEntity>> resourceUnits = PipelineRuntimeData.getServiceResourceUnit(service.getSvcId());
  ...
  private void processUnhealthyService(SpServiceRegistration service) {
    ...
    if (shouldDeleteService(service)) {
      ...
      if(!serviceRegistrations.contains(service)) {
        ...
        if(ElementServiceStats.elementServiceStats.containsKey(service.getSvcId())) {
          ElementServiceStats.elementServiceStats.get(service.getSvcId()).remove();
        }
      }
    }
